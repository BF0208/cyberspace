...
function init() {
  ...
  scene.fog = new THREE.FogExp2(0xbcbcbc, 0.05);
  ...
  for (let i = 1; i <= 15; i++) {
    loader.load(`images/image${i}.jpg`, (texture) => {
      ...
      mesh.position.set((Math.random() - 0.5) * 4, (Math.random() - 0.5) * 3, -i * 5);
      scene.add(mesh);
...
function animate() {
  ...
  camera.lookAt(new THREE.Vector3(currentX, currentY, scroll - 1));
